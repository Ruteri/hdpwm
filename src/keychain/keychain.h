...
struct KeychainDirectory {
	...
	KeychainDirectory(const KeychainDirectoryMeta &meta, KeychainDirectory *parent): meta(meta), parent(parent) {
...
class Keychain {
  ...
  public:
	...
	std::shared_ptr<KeychainDirectory> get_root_dir() const {
		...
		std::shared_ptr<KeychainDirectory> root = std::make_unique<KeychainDirectory>(KeychainDirectoryMeta{"/", ""}, nullptr);
		...
		root->dirs[0].entries.emplace_back(KeychainEntryMeta{"some password", "password to some site", {0}}, &(root->dirs[0]));
		root->entries.emplace_back(KeychainEntryMeta{"other password", "password to some other site", {1}}, root_raw_ptr);
		root->is_open = true;
		return root;
	}
};
